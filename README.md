...
- 商品作成／更新に必要な商品データ（タイトル、説明、画像、販売価格、定価、在庫数）
  - タイトルは投稿データのタイトルが送信される
  - 説明は投稿データの抜粋が送信される
  - 画像はアイキャッチ画像が設定されているときは、その画像。さらにACFカスタムフィールドでgalleryフィールドがあって、そのフィールド名がgalleryとなっているときは、その中の画像も加えられる。
  - 販売価格はACFカスタムフィールドでprcesグループが設定され、そのメンバーとしてsales_priceが設定されているときはその価格
  - 定価は上記のグループにlist_priceが設定されている場合はその価格
  - 在庫数はACFカスタムフィールドでquantityが設定されているときは、その数量が初期値として設定されるが、Shopifyでの販売履歴は反映されないので、初期設定以外では使用しないようにすること
- カート行情報（バリアントID、数量）
